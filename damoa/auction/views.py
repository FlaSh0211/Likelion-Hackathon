...
def mainA(request): # 등록된 게시글 페이지 !!!새로 수정된 곳 (paginator 적용)
    ...
    bids = Bid.objects.filter(userId_id = userid)
    context = {'post':post, 'posts':posts, 'username': mark_safe(json.dumps(username)),'userid':mark_safe(json.dumps(userid))}
...
def detail(request, id):
    ...
    post.read =True
    post.save()
    context = {'post':post, 'username': mark_safe(json.dumps(username))}
...
def detail2(request, id):
    ...
    post.save()
    context = {'post':post, 'username': mark_safe(json.dumps(username)),'userid':mark_safe(json.dumps(userid))}
    return render(request, 'auction/detail.html', context)
...
def writerinfo(request, id): # 0807 수정
    ...
    context = {'post':post, 'seller':seller, 'info':info, 'info2':info2, 'rateall':rateall, 'rate':rate}
